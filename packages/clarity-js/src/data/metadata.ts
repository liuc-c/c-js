...
export function start(): void {
  ...
  electron =
    ua.indexOf(Constant.Electron) > 0 ? BooleanFlag.True : BooleanFlag.False;
  ...
  config.lean =
    config.track && s.upgrade !== null
      ? s.upgrade === BooleanFlag.False
      : config.lean;
  ...
  if (navigator) {
    ...
    metric.max(
      Metric.DeviceMemory,
      Math.round((navigator as any).deviceMemory)
    );
    userAgentData();
  }
    ...
    uaData
      ...
      .then((ua) => {
        ...
        ua.brands?.forEach((brand) => {
          dimension.log(
            Dimension.Brand,
            brand.name + Constant.Tilde + brand.version
          );
        });
        dimension.log(Dimension.Model, ua.model);
        metric.max(
          Metric.Mobile,
          ua.mobile ? BooleanFlag.True : BooleanFlag.False
        );
      });
  } else {
...
export function metadata(cb: MetadataCallback, wait: boolean = true): void {
  ...
  if (data && (upgraded || wait === false)) {
    ...
    callbacks.push({ callback: cb, wait });
  }
}

export function id(): string {
  return data
    ? [data.uuid, data.sessionId, data.pageNum].join(Constant.Dot)
    : Constant.Empty;
}
...
export function save(): void {
  ...
  setCookie(
    Constant.SessionKey,
    [data.sessionId, ts, data.pageNum, upgrade, upload].join(Constant.Pipe),
    Setting.SessionExpire
  );
}
...
function track(u: User, consent: BooleanFlag = null): void {
  ...
  if (
    u.expiry === null ||
    Math.abs(end - u.expiry) >= Setting.CookieInterval ||
    u.consent !== consent ||
    u.dob !== dob
  ) {
    let cookieParts = [
      data.uuid,
      Setting.CookieVersion,
      end.toString(36),
      consent,
      dob,
    ];
    setCookie(
      Constant.CookieKey,
      cookieParts.join(Constant.Pipe),
      Setting.Expire
    );
  }
}
...
export function rid(v: string = "") {
  var k = "st_rid_" + config.d;

  if (!v) {
    v = generateId("rid", config.d, config.mark).toString();
  }
  if (v !== null) {
...
function sid(k: string = null, v: string = null) {
  if (!k) {
    k = "st_sid_" + config.d;
  }
  if (!v) {
    v = generateId("sid", config.d, config.mark).toString();
  }
...
function session(): Session {
  ...
  if (value) {
    ...
    if (
      parts.length >= 5 &&
      output.ts - num(parts[1]) < Setting.SessionTimeout
    ) {
      ...
      output.upload =
        parts.length >= 6
          ? `${Constant.HTTPS}${parts[5]}/${parts[4]}`
          : `${Constant.HTTPS}${parts[4]}`;
    }
  }
  return output;
}
...
function uuid(k: string = null, v: string = null) {
  if (!k) {
    k = "st_uuid_" + config.d;
  }
  if (!v) {
...
function setCookie(key: string, value: string, time: number): void {
  ...
  if (
    (config.track || value == Constant.Empty) &&
    ((navigator && navigator.cookieEnabled) ||
      supported(document, Constant.Cookie))
  ) {
    // Some browsers automatically url encode cookie values if they are not url encoded.
    // We therefore encode and decode cookie values ourselves.
    ...
    let expires = expiry
      ? Constant.Expires + expiry.toUTCString()
      : Constant.Empty;
    ...
    try {
      ...
      if (rootDomain === null) {
        ...
        for (let i = hostname.length - 1; i >= 0; i--) {
          rootDomain = `.${hostname[i]}${
            rootDomain ? rootDomain : Constant.Empty
          }`;
    ...
    document.cookie = rootDomain
      ? `${cookie}${Constant.Semicolon}${Constant.Domain}${rootDomain}`
      : cookie;
  }
}

function generatePv() {
  return generateId("pv", config.d, config.mark);
}


function generateId(mark = "id", uniqueId1, uniqueId2) {
  let timestamp = Date.now();
  let sequence = parseInt(mark, 36);
  ...
  let id =
    ...
    (random & 0xfffff);
  id = Math.abs(id);
  let length = 18 - id.toString().length;
  if (length < 0) {
