...
export function trigger(input: string): void {
    try {
        let parts = input && input.length > 0 ? input.split(/ (.*)/) : [Constant.Empty];
        let keyparts = parts[0].split(/\|(.*)/);
        let key = parseInt(keyparts[0]);
        let element = keyparts.length > 1 ?  keyparts[1] : Constant.Empty;
        let values = parts.length > 1 ? JSON.parse(parts[1]) : {};
        variables[key] = {};
        selectors[key] = {};
        hashes[key] = {};
